...
TEST_F(Llama3OutputParserTest, ParseToolCallOutputWithSingleToolCall) {
    ...
    ParsedOutput parsedOutput = ovms::test::parseWithStreamer(*llama3Tokenizer, *outputParserWithRegularToolParsing, generatedTokens, true);
...
TEST_F(Llama3OutputParserTest, ParseToolCallOutputNoToolsInTheRequest) {
    ...
    auto generatedTensor = llama3Tokenizer->encode(input, ov::genai::add_special_tokens(false)).input_ids;
    std::vector<int64_t> generatedTokens(generatedTensor.data<int64_t>(), generatedTensor.data<int64_t>() + generatedTensor.get_size());
    ParsedOutput parsedOutput = ovms::test::parseWithStreamer(*llama3Tokenizer, *outputParserWithRegularToolParsing, generatedTokens, false);
