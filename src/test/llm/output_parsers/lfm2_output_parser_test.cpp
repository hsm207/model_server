...
TEST_F(LFM2OutputParserTest, ParseToolCallOutputWithSingleToolCall) {
    ...
    for (auto& input : inputs) {
        ...
        ParsedOutput parsedOutput = ovms::test::parseWithStreamer(*lfm2Tokenizer, *outputParserWithRegularToolParsing, generatedTokens, true);
...
TEST_F(LFM2OutputParserTest, ParseToolCallOutputWithNoToolsInTheRequest) {
    ...
    for (auto& input : inputs) {
        ...
        std::vector<int64_t> generatedTokens(generatedTensor.data<int64_t>(), generatedTensor.data<int64_t>() + generatedTensor.get_size());
        ParsedOutput parsedOutput = ovms::test::parseWithStreamer(*lfm2Tokenizer, *outputParserWithRegularToolParsing, generatedTokens, false);
